...
# ---------------------------------------------------------------------------
# Royal 21 Table Management
# ---------------------------------------------------------------------------

@admin_bp.route('/royal21-tables')
@admin_required
def royal21_tables():
    user = get_current_user()
    tables = Royal21Table.query.order_by(Royal21Table.id.desc()).all()
    return render_template('admin/royal21_tables.html', user=user, tables=tables)


@admin_bp.route('/api/royal21/create-table', methods=['POST'])
@admin_required
def royal21_create_table():
    user = get_current_user()
    try:
        table = Royal21Table(
            table_name=request.form.get('table_name', 'Royal 21 Table'),
            ante=int(request.form.get('ante', 10)),
            min_buyin=int(request.form.get('min_buyin', 100)),
            max_buyin=int(request.form.get('max_buyin', 1000)),
            is_open=True,
            admin_id=user.id,
        )
    ...
    return redirect(url_for('admin.royal21_tables'))


@admin_bp.route('/api/royal21/open-table/<int:table_id>', methods=['POST'])
@admin_required
def royal21_open_table(table_id):
    table = Royal21Table.query.get_or_404(table_id)
    table.is_open = True
    db.session.commit()
    flash(f'Table "{table.table_name}" opened.', 'success')
    return redirect(url_for('admin.royal21_tables'))


@admin_bp.route('/api/royal21/close-table/<int:table_id>', methods=['POST'])
@admin_required
def royal21_close_table(table_id):
    table = Royal21Table.query.get_or_404(table_id)
    table.is_open = False
    db.session.commit()
    flash(f'Table "{table.table_name}" closed.', 'success')
    return redirect(url_for('admin.royal21_tables'))
