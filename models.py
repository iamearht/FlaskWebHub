...
class Royal21Table(db.Model):
    __tablename__ = 'royal21_tables'

    id = db.Column(db.Integer, primary_key=True)
    table_name = db.Column(db.String(100), nullable=False, default='Royal 21 Table')
    ante = db.Column(db.Integer, nullable=False, default=10)
    min_buyin = db.Column(db.Integer, nullable=False, default=100)
    max_buyin = db.Column(db.Integer, nullable=False, default=1000)
    is_open = db.Column(db.Boolean, nullable=False, default=True)
    admin_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    admin = db.relationship('User', backref='royal21_tables')
    seats = db.relationship('Royal21Seat', backref='table', cascade='all, delete-orphan')
...
class Royal21Seat(db.Model):
    __tablename__ = 'royal21_seats'

    id = db.Column(db.Integer, primary_key=True)
    table_id = db.Column(db.Integer, db.ForeignKey('royal21_tables.id'), nullable=False)
    seat_number = db.Column(db.Integer, nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    coins_escrowed = db.Column(db.Integer, nullable=False)
    joined_at = db.Column(db.DateTime, default=datetime.utcnow)

    user = db.relationship('User', backref='royal21_seats')

    __table_args__ = (
        db.UniqueConstraint('table_id', 'seat_number', name='uq_r21_table_seat'),
        db.UniqueConstraint('table_id', 'user_id', name='uq_r21_table_user'),
    )
