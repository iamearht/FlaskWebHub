...
- `admin_bp` - /admin/ (dashboard, rake-stats, lobby-rake, tournament-rake, tournament-payouts, coins, rakeback, jackpot, affiliate)
...
## Affiliate System
- 4 tiers: Bronze (2%), Silver (5%), Gold (8%), Platinum (12%) - all configurable from admin
- Tier based on total rake generated by referred players (each player contributes half the match rake)
- Commission = referred player's rake share x tier commission %
- Affiliate dashboard shows current tier, progress bar to next tier, tier breakdown grid
- Admin can configure tier names, rake thresholds, and commission percentages
